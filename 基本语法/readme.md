...
* [类](#类)
   ...
   * [cpp11新特性](#cpp11新特性)
   * [双引号和尖括号区别](#双引号和尖括号区别)
   * [如何定义一个只在堆（栈）上生成对象的类](#如何定义一个只在堆或栈上生成对象的类)
* [编译与底层](#编译与底层)
   * [malloc 的原理  brk 系统调用  mmap 系统调用](#malloc的原理)
   * [cpp内存管理](#cpp内存管理)
   * [extern c](#extern-c)
   * [内存泄漏](#内存泄漏)
   * [未初始化全局变量与common块](#未初始化全局变量与common块)
<!-- GFM-TOC -->
...
**2、static_cast**
    用于各种**隐式转换**，比如非 const 转 const，void* 转指针等, static_cast 能用于多态向上转化（子类向父类转换），如果向下转能成功但是不安全，结果未知;  不执行运行时类型检查
    
**3、dynamic_cast**
    用于**动态类型转换**。只能用于**含有虚函数的类**，用于类层次间的向上和向下转化。只能转指针或引用，对于不明确的指针的转换返回nullptr,但不引发异常。
...
**4、reinterpret_cast**
     几乎什么都可以转，比如将 int 转指针，可能会出问题，尽量少用; 但是不能丢掉const、volatile、_unaligned特性。
...
---- | ---
 ...
 返回的指针不用强转   | 返回的指针需要强转 
 可以重载   |  不能重载
 new和delete底层调用了malloc, 对于new是先内存分配，再调用构造函数（创建对象）；对于delete是先析构函数（清理资源）再内存释放。
...
### 只在堆上
    方法：将析构函数设置成私有
    原因：编译器管理栈上得对象得生命周期，编译器在为类对象分配栈空间时，会先检查类的析构函数，如果类的析构函数不可访问，则不能在栈上创建对象
### 只在栈上
    方法：将new华为delete重载为私有
    原因：在堆上生成对象，使用new关键字操作，第一阶段：使用new在堆上寻找可用内存，分配给对象；第二阶段：调用构造函数生成对象。若将new设置为私有，第一阶段无法完成，不能在堆上生成对象。
...
Tips：符号修饰是为了在编译和链接阶段，识别由于重载，名称空间等造成的同名函数

extern "C" 使用
```
#ifdef __cplusplus  // 使用 __cplusplus是为了兼容C语言和C++定义的两套头文件，使用条件宏来判断当前编译单元是不是C++代码
extern "C" {
#endif

void *memset(void *, int, size_t); //避免将memset进行符号修饰，从而导致无法和C语言库进行链接
...
分类 | 内容
---- | ---
堆内存泄漏 |   内存指的是程序运行中根据需要分配通过 malloc,realloc new 等从堆中分配的一块内存，再是完成后必须通过调用对应的 free 或者 delete 删掉。如果 程序的设计的错误导致这部分内存没有被释放，那么此后这块内存将不会被使用，就会产生 Heap Leak
系统资源泄露 | 指程序使用系统分配的资源比如 Bitmap,handle ,SOCKET 等没有使用相应的函数释放掉，导致系统资源的浪费，严重可导致系统 效能降低，系统运行不稳定。
没有将基类的析构函数定义为虚函数  |  基类指针指向子类对象时，如果基类的析构函 数不是 virtual，那么子类的析构函数将不会被调用，子类的资源没有正确是释放，因此造成内 存泄露。

## 未初始化全局变量与common块
编译器会把未初始化的全局变量标记未一个common类型的变量，而不是把它当成未初始化的局部变量，为其在bss段分配内存

在编译阶段，编译器无法知道该弱符号的大小，可能其它目标文件中该符号所占内存空间比本单元弱符号所占内存空间大，所以无法分配空间。

但是链接器读取所有输入目标文件以后，任何一个弱符号的最终大小都可以确定了，所以可以在最终输出文件的bss段为其分配空间
